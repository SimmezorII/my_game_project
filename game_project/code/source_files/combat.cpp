...
inline void setSquareField(vector<sprite>& sprite_list, field& fieldRef, int nx,
  ...
  for (int i = 0; i < sum1; i++) {
    ...
    x = (tile_width) + fx - ((tile_width / 2) * (i)) +
        row * ((tile_width / 2) * row) + (row * tile_width) -
        (targetRange * (tile_width / 2));
    y = (-tile_height) + fy - ((tile_height / 2) * (i)) +
        row * ((tile_height / 2) * row) + (targetRange * (tile_height / 2));
  ...
  while (start < (sum1 + sum2)) {
    ...
    x = fx + ((tile_width / 2) * (start - sum1)) -
        line * ((tile_width / 2) * line) - (line * tile_width) +
        (targetRange * (tile_width / 2));
    y = fy + ((tile_height / 2) * (start - sum1)) -
        line * ((tile_height / 2) * line) - (targetRange * (tile_height / 2));
...
inline void setTriangleField(vector<sprite>& sprite_list, field& fieldRef,
  ...
  if (dir == UP) {
    ...
    while (start < (sum1 + sum2)) {
      ...
      x = fx + -1 * (((tile_width / 2) * (start - sum1)) -
                     line * ((tile_width / 2) * line) - (line * tile_width) +
                     (targetRange * (tile_width / 2)));
      y = fy + ((tile_height / 2) * (start - sum1)) -
          line * ((tile_height / 2) * line) - (targetRange * (tile_height / 2));
  ...
  if (dir == DOWN) {
    ...
    while (start < (sum1 + sum2)) {
      ...
      x = fx + 1 * (((tile_width / 2) * (start - sum1)) -
                    line * ((tile_width / 2) * line) - (line * tile_width) +
                    (targetRange * (tile_width / 2)));
      y = fy + -1 * (((tile_height / 2) * (start - sum1)) -
                     line * ((tile_height / 2) * line) -
                     (targetRange * (tile_height / 2)));
...
inline void setConeField(vector<sprite>& sprite_list,field& fieldRef, int nx, int ny, Direction dir) {
  int targetRange = fieldRef.range;
...
inline void SetField(game_state &GameState, field& fieldRef, int nx, int ny, int field_type,
                     int tile_color) {
...
inline void InitEnemyFields(game_state &GameState) {
  for (size_t i = 0; i < enemy_list.size(); i++) {
    ...
    SetField(GameState, enemy_list[i].move_field, enemy_list[i].pEntity->x,
             enemy_list[i].pEntity->y, SQUARE, RED_TILE);
...
inline void InitCombat(game_state &GameState) {
  ...
  SetField(GameState,position_field, 512, 512, NON_ISO_SQUARE, GREEN_TILE);
}
