...
inline void InitGui() { new_game_entity = &new_e; }
...
inline void RenderEntityInfo() {
  ...
  if (combatant_selected > -1) {
    ...
    temp.y =
        combatant_list[combatant_selected].pEntity->offset_rect.y - FONT_SIZE;
    temp.width = 100;  // Size of max string
    temp.height = FONT_SIZE;

    GuiLabel(temp,
             combatant_list[combatant_selected].pEntity->sprite->name.c_str());
    ...
    for (size_t i = 0; i < enemy_list.size(); i++) {
      if (combatant_selected > -1) {
        ...
        s = to_string((int)enemy_list[i].pEntity->entity_stats.current_hp) +
            "/" + to_string((int)enemy_list[i].pEntity->entity_stats.max_hp);
...
inline void DrawGui(game_state &GameState) {
  int n = 0;
  ...
  GuiSpinner({gui_temp.x, gui_temp.y, gui_temp.width, gui_temp.height},
             "sprite id", &game_entity->sprite->ID, 1,
  ...
  GuiTextBox(
      ...
      strdup(game_entity->sprite->name.c_str()), 12, false);
  n++;
  ...
  GuiSpinner({gui_temp.x, gui_temp.y, gui_temp.width, gui_temp.height},
             "sprite x", &game_entity->sprite->x, 0, 2000, ToggleSpriteX);
  n++;
  ...
  gui_temp.y = 12 + SpriteGroupBoxRect.y + (n * 20);
  GuiSpinner({gui_temp.x, gui_temp.y, gui_temp.width, gui_temp.height},
             "sprite y", &game_entity->sprite->y, 0, 2000, ToggleSpriteY);
  n++;
  ...
  gui_temp.y = 12 + SpriteGroupBoxRect.y + (n * 20);
  GuiSpinner({gui_temp.x, gui_temp.y, gui_temp.width, gui_temp.height},
             "sprite w", &game_entity->sprite->w, 0, 2000, false);
  n++;
  game_entity->sprite->w = GuiSliderBar(
      {gui_temp.x + 92, gui_temp.y, gui_temp.width, gui_temp.height}, "",
      TextFormat("%i", game_entity->sprite->w), game_entity->sprite->w, 0,
      2000);

  gui_temp.y = 12 + SpriteGroupBoxRect.y + (n * 20);
  GuiSpinner({gui_temp.x, gui_temp.y, gui_temp.width, gui_temp.height},
             "sprite h", &game_entity->sprite->h, 0, 2000, false);
  n++;
  game_entity->sprite->h = GuiSliderBar(
      {gui_temp.x + 92, gui_temp.y, gui_temp.width, gui_temp.height}, "",
      TextFormat("%i", game_entity->sprite->h), game_entity->sprite->h, 0,
      2000);

  gui_temp.y = 14 + SpriteGroupBoxRect.y + (n * 20);
  GuiSpinner({gui_temp.x, gui_temp.y, gui_temp.width, gui_temp.height},
             "offset_x", &game_entity->sprite->offset_x, -800, 2000,
             ToggleSpriteOffsetX);
  n++;
  ...
  gui_temp.y = 12 + SpriteGroupBoxRect.y + (n * 20);
  GuiSpinner({gui_temp.x, gui_temp.y, gui_temp.width, gui_temp.height},
             "offset_y", &game_entity->sprite->offset_y, -800, 2000,
             ToggleSpriteOffsetY);
  n++;
  ...
  if (GuiDropdownBox(
          ...
          GameState.AllPNGList.c_str(), &game_entity->sprite->index,
          dropDown001EditMode)) {
  ...
  combat = GuiToggle({gui_temp.x + 2, gui_temp.y,
  ...
  draw_fields = GuiToggle({gui_temp.x + 2, gui_temp.y,
...
inline void RenderNewEntity(game_state &GameState) {
  ...
  if (temp_sprite_rec.height <= 32) {
    ...
    temp_sprite_rec.y =
        GetMousePosition().y - (temp_sprite_rec.height) + tile_height / 2;
  }
...
inline void AddNewEntity( vector<entity> entity_list,int x, int y, int screen_x, int screen_y) {
...
inline void CreateNewEntity( vector<entity> & entity_list) {
  pos p;
  ...
  if (IsMouseButtonReleased(MOUSE_LEFT_BUTTON)) {
    ...
    if (PLACING_ENTITY) {
      ...
      if (p.x != -1 && p.y != -1) {
        ...
        AddNewEntity(entity_list,p.x, p.y, p.screen_x,
                     p.screen_y - (CURRENT_LAYER * (GAME_TILE_HEIGHT / 2)));
...
inline void DrawActionGui() {
  ...
  DrawRectangle(ActionMenuRects[0].x, ActionMenuRects[0].y,
                ActionMenuRects[0].width + 24, ActionMenuRects[0].height,
                WHITE);

  GuiGroupBox({ActionMenuRects[0].x, ActionMenuRects[0].y,
               ActionMenuRects[0].width + 24, ActionMenuRects[0].height},
              "Action Menu");

  if (precombat == true) {
    ActionButton[0] =
        GuiButtonEx(ActionMenuRects[1], "Ready", action_target_rect, KEY_X);

    ActionButton[1] =
        GuiButtonEx(ActionMenuRects[2], "Escape", action_target_rect, KEY_X);

    ActionButton[2] =
        GuiButtonEx(ActionMenuRects[3], "End", action_target_rect, KEY_X);
    ...
    ActionButton[0] =
        GuiButtonEx(ActionMenuRects[1], "Move", action_target_rect, KEY_X);

    ActionButton[1] =
        GuiButtonEx(ActionMenuRects[2], "Attack", action_target_rect, KEY_X);
...
inline void RenderLog() {
        ...
        if (rendernum > 89 - LogScrollCounter - 1) {
          GuiLabel({gamescreen_offset_x + LogInfoRect.x,
                    gamescreen_offset_y + LogInfoRect.y +
                        ((i - LogScrollCounter) * LogInfoRect.height) -
                        (LogInfoRect.height * 1),
                    LogInfoRect.width, LogInfoRect.height},
                   log_lines[(currentline_log - 1 - rendernum)].c_str());
          rendernum++;
        } else {
          rendernum++;
        }
  ...
  LogScrollCounter =
      GuiScrollBar(ScrollBar, LogScrollCounter, 100 - currentline_log, 89);
}
...
inline void RenderDebugLog() {
      ...
      if (rendernum < 89) {
        if (rendernum > 89 - DebugLogScrollCounter - 1) {
          GuiLabel(
              {DebugLogInfoRect.x,
               DebugLogInfoRect.y +
                   ((i - DebugLogScrollCounter) * DebugLogInfoRect.height) -
                   (DebugLogInfoRect.height * 1),
               DebugLogInfoRect.width, DebugLogInfoRect.height},
              debuglog_lines[(currentline_debuglog - 1 - rendernum)].c_str());
          rendernum++;
        } else {
          rendernum++;
        }
  ...
  DebugLogScrollCounter = GuiScrollBar(DebugScrollBar, DebugLogScrollCounter,
                                       100 - currentline_debuglog, 89);
}
