...
RAYGUIDEF bool GuiButtonEx(Rectangle bounds, const char *text, Rectangle gamerect, KeyboardKey key);		// ADDED BUTTON
...
// Button control, returns true when clicked
bool GuiButtonEx(Rectangle bounds, const char *text, Rectangle gamerect, KeyboardKey key)
{
	GuiControlState state = guiState;
	bool pressed = false;
	...
	if ((state != GUI_STATE_DISABLED) && !guiLocked)
	{
		...
		// Check button state
		if (CheckCollisionRecs(gamerect, bounds))
		{
			if (IsKeyDown(key))
			{
